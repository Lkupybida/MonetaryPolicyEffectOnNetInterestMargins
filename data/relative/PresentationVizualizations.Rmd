...
theme_custom <- function() {
  theme(
    ...
    panel.grid.minor = element_line(color = background, linetype = "dotted"),
...
p <- ggplot(combined_data, aes(x = Date, y = IR, color = Source)) +
  ...
  labs(title = "Time Series Plot of IR for Interbank and Policy",
       ...
       y = chosen_column) +
  theme_custom() +
  scale_color_manual(values = c(background, complement))
...
display_plot(name)
  ...
  #   axis.title = element_text(size = 2),          # Smaller axis title size
  #   legend.title = element_text(size = 5),         # Smaller legend title size
  #   legend.text = element_text(size = 4)           # Smaller legend label size
...
```{r}
plot_median_quartiles("~/Documents/GitHub/MonetaryPolicyEffectOnNetInterestMargins/data/relative/ovdp_to_total_assets.csv", "~/Documents/GitHub/MonetaryPolicyEffectOnNetInterestMargins/plots/ovdp.png")
```
